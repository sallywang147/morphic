...
pub fn annot_rcs(
    ...
    GLOBAL_INDEX.store(0, Ordering::SeqCst);
    //let index = 0;
    ...
    let funcs = IdVec::from_vec(
        program
            ...
            .map(|(func_id, func)| {
                ...
                //threading index: pass on to annot_rc
                //rc_specialized: splitting into multiple ASTs
                //next pass if rc gets split, we just copy the index
                progress.update(1);
                annot
            })
            .collect(),
